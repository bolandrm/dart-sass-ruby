  ...
  class EngineTest < MiniTest::Test
    ...
    def test_load_paths
      ...
      assert_equal ".hi {\n  width: 30px;\n}", Engine.new(
        ...
        load_paths: [temp_file_path("included_1"), temp_file_path("included_2")]
      ).render
    end
    ...
    def test_global_load_paths
      ...
      ::DartSass.load_paths << temp_file_path("included_1")
      ::DartSass.load_paths << temp_file_path("included_2")

      assert_equal ".hi {\n  width: 30px;\n}", Engine.new(
        File.read(temp_file_path("styles.scss"))
      ).render
      ::DartSass.load_paths.clear
    end

    def test_env_load_paths
      expected_load_paths = ["included_1", "included_2"]
      ::DartSass.instance_eval { @load_paths = nil }
      ENV["SASS_PATH"] = expected_load_paths.join(File::PATH_SEPARATOR)
      assert_equal expected_load_paths, ::DartSass.load_paths
      ::DartSass.load_paths.clear
    end
    ...
    def test_empty_template_encoding_matches_input
      input = +""
      input = input.force_encoding("ISO-8859-1")
      output = Engine.new(input).render
      assert_equal input.encoding, output.encoding
    end
